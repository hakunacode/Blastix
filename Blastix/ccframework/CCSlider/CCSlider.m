...
{
	[super initFromNormalImage:@"option_slider.png" 
				 selectedImage:@"option_slider_bar.png" 
				 disabledImage:nil target:t selector:sel];
	if(SCALE_SCREEN_WIDTH == 1) _max = 260;
	else _max = 270;
...
-(float) value
{
	return (position_.x+(_max * SCALE_SCREEN_WIDTH /2))/(_max * SCALE_SCREEN_WIDTH);
}

-(void) setValue:(float)val
{
	[self setPosition:ccp(val*_max*SCALE_SCREEN_WIDTH - (_max * SCALE_SCREEN_WIDTH/2), position_.y)];
}
...
-(void) ccTouchMoved:(UITouch *)touch withEvent:(UIEvent *)event
{
	...
	if (x < -_max * SCALE_SCREEN_WIDTH /2)
		_thumb.position = ccp(-_max * SCALE_SCREEN_WIDTH/2, 0);
	else if (x > _max * SCALE_SCREEN_WIDTH/2)
		_thumb.position = ccp(_max * SCALE_SCREEN_WIDTH/2, 0);
	else
...
-(id) initWithTarget:(id)t selector:(SEL)sel
{
	if( (self = [super init]) ) {
		...
		if( _touchLogic ) {
			...
			barImage_ = [CCSprite spriteWithFile:@"option_slider_bar.png"];
